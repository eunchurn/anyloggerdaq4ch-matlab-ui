...
% Last Modified by GUIDE v2.5 30-Aug-2012 03:50:39
...
%                     end
                    ...
                    mssgs{end+1}=['check connection or set lower buffersize'];
                    set(handles.mssgbox,'String',mssgs);set(handles.mssgbox,'Value',length(mssgs));
...
buffersizeMB=str2num(get(handles.buffersizetag,'String'));
autosave=get(handles.autosavebtn,'Value');
autosavetime=str2num(get(handles.autosavetime,'String'));
skipid=0;
...
%                                             data=adval*adgain;
                                    ...
                                    if autosave
                                        if adcount>=autosavetime*Fs
                                            ...
                                            set(handles.savedatabtn,'UserData',1);
                                            skipid=1;
                                            HRDAQ4CH_V1('savedatabtn_Callback',hObject,eventdata,guidata(hObject));
%                                             set(handles.startbtn,'Value',0);
                                            HRDAQ4CH_V1('startbtn_Callback',hObject,eventdata,guidata(hObject));
                                            pause(2);
                                            HRDAQ4CH_V1('startbtn_Callback',hObject,eventdata,guidata(hObject));
                                        ...
                                        try
                                            if get(handles.displaymodebtn,'Value')
                                                ...
                                                if get(handles.ydbbtn,'Value')
                                                    set(plotHandle,'YData',pow2db(Pxx/Fs),'XData',F);
...
automode=get(handles.savedatabtn,'UserData');
...
set(handles.savedatabtn,'UserData',0);
...
%                 set(0,'DefaultAxesColorOrder',corder);
                ...
                if get(handles.ydbbtn,'Value')
                    hpsd = plot(F,pow2db(Pxx(:,selch)/Fs),'.-','Parent',handles.mainaxes);
                    if get(handles.xlogbtn,'Value')
                        set(handles.mainaxes,'XScale','log');
...
% --- Executes on button press in autosavebtn.
function autosavebtn_Callback(hObject, eventdata, handles)
% hObject    handle to autosavebtn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of autosavebtn
if get(handles.autosavebtn,'Value')
    set(handles.autosavebtn,'String','ON');
else
    set(handles.autosavebtn,'String','OFF');
end


function autosavetime_Callback(hObject, eventdata, handles)
% hObject    handle to autosavetime (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of autosavetime as text
%        str2double(get(hObject,'String')) returns contents of autosavetime as a double


% --- Executes during object creation, after setting all properties.
function autosavetime_CreateFcn(hObject, eventdata, handles)
% hObject    handle to autosavetime (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
